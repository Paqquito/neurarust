...
where
    T: Mul<Output = T> + Copy + Clone + Debug + Default + Zero + One + Sum + AddAssign + 'static + PartialEq,
{
    ...
    if a_guard.device != b_guard.device {
        return Err(NeuraRustError::UnsupportedOperation(
            format!("Cannot multiply tensors on different devices: {:?} and {:?}", a_guard.device, b_guard.device)
        ));
    }
    let device = a_guard.device; 
    if device != StorageDevice::CPU {
         return Err(NeuraRustError::UnsupportedOperation(
            format!("Multiplication is currently only supported on CPU, not {:?}", device)
        ));
    }
...
where
     T: Mul<Output = T> + AddAssign + Copy + Clone + 'static + Default + Debug + Zero + One + Sum + PartialEq,
{
...
mod tests {
    ...
    where 
        T: Clone + Debug + PartialEq + Zero + One + AddAssign + Copy + Mul<Output = T> + Default + Sum + 'static
    { 
    ...
    fn test_mul_tensors_ok() {
        ...
        let trait_res_cpu_data = trait_res_buffer_arc.cpu_data().expect("Trait result tensor not on CPU");
        assert_eq!(trait_res_cpu_data.as_slice(), expected_data.as_slice(), "Data mismatch (trait)");
    ...
    fn test_mul_broadcasting() {
        ...
        let scalar_res_cpu_data = scalar_res_buffer_arc.cpu_data().expect("Scalar mul result not on CPU");
        assert_eq!(scalar_res_cpu_data.as_slice(), expected_scalar_mul.as_slice());
        ...
        let scalar_rev_res_cpu_data = scalar_rev_res_buffer_arc.cpu_data().expect("Scalar mul reverse result not on CPU");
        assert_eq!(scalar_rev_res_cpu_data.as_slice(), expected_scalar_mul.as_slice());
    }
} 