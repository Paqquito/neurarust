...
mod tests {
    ...
    fn test_div_tensors_ok() -> Result<(), NeuraRustError> {
        let a = crate::tensor::from_vec_f32(vec![10.0, 20.0], vec![2])?;
        let b = crate::tensor::from_vec_f32(vec![2.0, 5.0], vec![2])?;
    ...
    fn test_div_by_zero() -> Result<(), NeuraRustError> {
        let a = crate::tensor::from_vec_f32(vec![10.0], vec![1])?;
        let b = crate::tensor::from_vec_f32(vec![0.0], vec![1])?;
        let result = div_op(&a, &b);
    ...
    fn test_div_broadcasting() -> Result<(), NeuraRustError> {
        let matrix = crate::tensor::from_vec_f32(vec![10.0, 20.0, 30.0, 40.0], vec![2, 2])?;
        let row_vector = crate::tensor::from_vec_f32(vec![2.0, 5.0], vec![1, 2])?;
    ...
    fn test_div_backward_simple() -> Result<(), GradCheckError> {
        ...
        let output_grad = crate::tensor::from_vec_f32(vec![1.0, 1.0], output_shape)?;
    ...
    fn test_div_backward_broadcast() -> Result<(), GradCheckError> {
        ...
        let output_grad = crate::tensor::from_vec_f32(vec![1.0, 1.0, 1.0, 1.0], output_shape)?;
    ...
    fn test_div_backward_with_zero_divisor() -> Result<(), NeuraRustError>{
        let a = crate::tensor::from_vec_f32(vec![10.0], vec![1])?;
        a.set_requires_grad(true)?;
        let b = crate::tensor::from_vec_f32(vec![0.0001], vec![1])?;
        b.set_requires_grad(true)?;
        let output_grad = crate::tensor::from_vec_f32(vec![1.0], vec![1])?;
