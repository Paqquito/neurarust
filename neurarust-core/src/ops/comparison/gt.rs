...
/// use neurarust_core::ops::comparison::gt_op;
/// let a = Tensor::new(vec![1.0, 2.0, 3.0], vec![3]).unwrap();
/// let b = Tensor::new(vec![2.0, 2.0, 2.0], vec![3]).unwrap();
/// let gt = gt_op(&a, &b).unwrap();
/// assert_eq!(gt.get_bool_data().unwrap(), vec![false, false, true]);
/// ```
pub fn gt_op(a: &Tensor, b: &Tensor) -> Result<Tensor, NeuraRustError> {
    let a_guard = a.read_data();
    let b_guard = b.read_data();

    if a_guard.device != StorageDevice::CPU || b_guard.device != StorageDevice::CPU {
        return Err(NeuraRustError::DeviceMismatch {
            operation: "gt_op".to_string(),
    ...
    if result_data_vec.len() != numel {
        return Err(NeuraRustError::InternalError(format!(
            "gt_op: Output vec len {} mismatch with expected numel {}",
...
mod tests {
    ...
    fn test_gt_op_basic() {
        ...
        let out = gt_op(&a, &b).unwrap();
        assert_eq!(out.dtype(), DType::Bool);
        assert_eq!(out.get_bool_data().unwrap(), vec![false, false, true]);
    }
} 