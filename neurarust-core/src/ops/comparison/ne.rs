...
/// use neurarust_core::ops::comparison::ne_op;
/// let a = Tensor::new(vec![1.0, 2.0, 3.0], vec![3]).unwrap();
/// let b = Tensor::new(vec![1.0, 0.0, 3.0], vec![3]).unwrap();
/// let ne = ne_op(&a, &b).unwrap();
/// assert_eq!(ne.get_bool_data().unwrap(), vec![false, true, false]);
/// ```
