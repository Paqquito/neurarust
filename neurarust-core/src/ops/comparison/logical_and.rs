...
/// Effectue un ET logique élément par élément entre deux tenseurs Bool.
pub fn logical_and_op(a: &Tensor, b: &Tensor) -> Result<Tensor, NeuraRustError> {
    ...
    if a_guard.dtype != DType::Bool || b_guard.dtype != DType::Bool {
        return Err(NeuraRustError::UnsupportedOperation(
            format!("logical_and_op requires Bool tensors, got {:?} and {:?}", a_guard.dtype, b_guard.dtype)
        ));
    }
