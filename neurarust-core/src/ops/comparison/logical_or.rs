...
/// Effectue un OU logique élément par élément entre deux tenseurs Bool.
pub fn logical_or_op(a: &Tensor, b: &Tensor) -> Result<Tensor, NeuraRustError> {
    let a_guard = a.read_data();
    let b_guard = b.read_data();
    if a_guard.device != StorageDevice::CPU || b_guard.device != StorageDevice::CPU {
        return Err(NeuraRustError::DeviceMismatch {
            operation: "logical_or_op".to_string(),
    ...
    if a_guard.dtype != DType::Bool || b_guard.dtype != DType::Bool {
        return Err(NeuraRustError::UnsupportedOperation(
            format!("logical_or_op requires Bool tensors, got {:?} and {:?}", a_guard.dtype, b_guard.dtype)
        ));
    }
    ...
    if result_data_vec.len() != numel {
        return Err(NeuraRustError::InternalError(format!(
            "logical_or_op: Output vec len {} mismatch with expected numel {}",
...
mod tests {
    ...
    fn test_logical_or_basic() {
        ...
        let result = logical_or_op(&t1, &t2).unwrap();
        assert_eq!(result.get_bool_data().unwrap(), vec![true, true, true, false]);
    }
    #[test]
    fn test_logical_or_broadcast() {
        ...
        let t2 = Tensor::new_bool(vec![true, true, false, false], vec![2, 2]).unwrap();
        let result = logical_or_op(&t1, &t2).unwrap();
        assert_eq!(result.get_bool_data().unwrap(), vec![true, true, false, false]);
    }
    #[test]
    fn test_logical_or_scalar_broadcast() {
        let t1 = Tensor::new_bool(vec![false], vec![1]).unwrap();
        let t2 = Tensor::new_bool(vec![false, true, true], vec![3]).unwrap();
        let result = logical_or_op(&t1, &t2).unwrap();
        assert_eq!(result.get_bool_data().unwrap(), vec![false, true, true]);
    }
    #[test]
    fn test_logical_or_type_error() {
        ...
        let result = logical_or_op(&t1, &t2);
