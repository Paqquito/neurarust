...
mod tests {
    ...
    use crate::tensor::create::{zeros, from_vec_f32, from_vec_f64, from_vec_i32};
    ...
    fn create_arc_param_with_grad(
        ...
        requires_grad: bool,
    ) -> Result<Arc<RwLock<Parameter>>, NeuraRustError> {
        ...
        if let Some(grad_data_vec) = grad_data_opt {
            let grad_tensor_val: Tensor = match dtype {
                DType::F32 => from_vec_f32(grad_data_vec, shape.clone())?,
                DType::F64 => from_vec_f64(
        ...
        Ok(Arc::new(RwLock::new(Parameter::new(tensor, Some("test_param".to_string())))))
    }

    // Helper pour créer un Parameter I32 pour les tests d'erreur de type
    fn create_arc_param_i32_grad() -> Result<Arc<RwLock<Parameter>>, NeuraRustError> {
        let tensor_for_param = zeros(&[1])?;
        let _ = tensor_for_param.set_requires_grad(true);
        let grad_i32 = from_vec_i32(vec![10], vec![1])?;
        tensor_for_param.data.write().unwrap().grad = Some(grad_i32);
        Ok(Arc::new(RwLock::new(Parameter::new(tensor_for_param, Some("param_i32".to_string())))))
    }

    #[test]
    fn test_clip_grad_value_f32() -> Result<(), NeuraRustError> {
        let param_arc1 = create_arc_param_with_grad(vec![2, 2], DType::F32, Some(vec![-5.0, 2.0, 3.0, -0.5]), true)?;
    ...
    fn test_clip_grad_value_f64() -> Result<(), NeuraRustError> {
        let param_arc1 = create_arc_param_with_grad(vec![2, 2], DType::F64, Some(vec![-5.0, 2.0, 3.0, -0.5]), true)?;
        let params_arcs = vec![param_arc1.clone()];
    ...
    fn test_clip_grad_value_no_grad() -> Result<(), NeuraRustError> {
        let param_no_grad_arc = create_arc_param_with_grad(vec![1], DType::F32, None, true)?;
        let param_with_grad_arc = create_arc_param_with_grad(vec![1], DType::F32, Some(vec![10.0]), true)?;
    ...
    fn test_clip_grad_value_already_in_bounds() -> Result<(), NeuraRustError> {
        let param_arc1 = create_arc_param_with_grad(vec![2], DType::F32, Some(vec![-0.5, 0.7]), true)?;
        let params_arcs = vec![param_arc1.clone()];
    ...
    fn test_clip_grad_value_negative_clip_value() {
        let param_arc1 = create_arc_param_with_grad(vec![2], DType::F32, Some(vec![-0.5, 0.7]), true).unwrap();
    ...
    fn test_clip_grad_norm_basic_clipping_l2() -> Result<(), NeuraRustError> {
        let param_arc1 = create_arc_param_with_grad(vec![2], DType::F32, Some(vec![3.0, 4.0]), true)?; 
        let param_arc2 = create_arc_param_with_grad(vec![1], DType::F32, Some(vec![12.0]), true)?;   
        ...
        assert_relative_eq!(grad1_data[0], 3.0 * clip_coef as f32, epsilon = 1e-6);
        assert_relative_eq!(grad1_data[1], 4.0 * clip_coef as f32, epsilon = 1e-6);
        assert_relative_eq!(grad2_data[0], 12.0 * clip_coef as f32, epsilon = 1e-6);
        
        let mut new_total_norm_pow_p: f32 = 0.0;
        for val in &grad1_data { new_total_norm_pow_p += val.abs().powf(norm_type as f32); }
        for val in &grad2_data { new_total_norm_pow_p += val.abs().powf(norm_type as f32); }
        let new_total_norm = new_total_norm_pow_p.powf(1.0/norm_type as f32);
        assert_relative_eq!(new_total_norm, max_norm as f32, epsilon = 1e-5); 
        Ok(())
    }

    #[test]
    fn test_clip_grad_norm_no_clipping_needed_l2() -> Result<(), NeuraRustError> {
        let param_arc1 = create_arc_param_with_grad(vec![2], DType::F32, Some(vec![1.0, 2.0]), true)?;
        let params_arcs = vec![param_arc1.clone()];
        let max_norm = 3.0;
        let norm_type = 2.0;

        let calculated_total_norm = clip_grad_norm_(params_arcs.into_iter(), max_norm, norm_type)?;
        assert_relative_eq!(calculated_total_norm as f32, 5.0_f32.sqrt(), epsilon = 1e-6);
    ...
    fn test_clip_grad_norm_l1_norm() -> Result<(), NeuraRustError> {
        let param_arc1 = create_arc_param_with_grad(vec![2], DType::F32, Some(vec![-2.0, 3.0]), true)?;
        let params_arcs = vec![param_arc1.clone()];
        let max_norm = 2.5;
        let norm_type = 1.0;
        ...
        let grad1_data = param_arc1.read().unwrap().tensor.data.read().unwrap().grad.as_ref().unwrap().get_f32_data()?;
        assert_relative_eq!(grad1_data[0], -2.0 * clip_coef as f32, epsilon = 1e-6);
        assert_relative_eq!(grad1_data[1], 3.0 * clip_coef as f32, epsilon = 1e-6);
        
        let mut new_total_norm_l1: f32 = 0.0;
        for val in &grad1_data { new_total_norm_l1 += val.abs(); }
        assert_relative_eq!(new_total_norm_l1, max_norm as f32, epsilon = 1e-5);
        Ok(())
    }

    #[test]
    fn test_clip_grad_norm_with_no_grad_param() -> Result<(), NeuraRustError> {
        let param_with_grad_arc = create_arc_param_with_grad(vec![2], DType::F32, Some(vec![6.0, 8.0]), true)?;
        let param_no_grad_arc = create_arc_param_with_grad(vec![1], DType::F32, None, true)?;
        ...
        assert_relative_eq!(grad1_data[0], 6.0 * clip_coef as f32, epsilon = 1e-6);
        assert_relative_eq!(grad1_data[1], 8.0 * clip_coef as f32, epsilon = 1e-6);
    ...
    fn test_clip_grad_norm_all_grads_zero() -> Result<(), NeuraRustError> {
        let param_arc1 = create_arc_param_with_grad(vec![2], DType::F32, Some(vec![0.0, 0.0]), true)?;
        let params_arcs = vec![param_arc1.clone()];
        let max_norm = 5.0;
        let norm_type = 2.0;
    ...
    fn test_clip_grad_norm_mixed_dtypes_f32_f64() -> Result<(), NeuraRustError> {
        let param_f32_arc = create_arc_param_with_grad(vec![2], DType::F32, Some(vec![3.0, 4.0]), true)?;
        let param_f64_arc = create_arc_param_with_grad(vec![1], DType::F64, Some(vec![12.0]), true)?;   
        ...
        assert_relative_eq!(grad_f32_data[0], 3.0 * clip_coef as f32, epsilon = 1e-6);
        assert_relative_eq!(grad_f32_data[1], 4.0 * clip_coef as f32, epsilon = 1e-6);

        let grad_f64_data = param_f64_arc.read().unwrap().tensor.data.read().unwrap().grad.as_ref().unwrap().get_f64_data()?;
        assert_relative_eq!(grad_f64_data[0], 12.0 * clip_coef, epsilon = 1e-6); 
        Ok(())
    }

    #[test]
    fn test_clip_grad_norm_invalid_max_norm() {
        let param_arc1 = create_arc_param_with_grad(vec![1], DType::F32, Some(vec![1.0]), true).unwrap();
        let params_arcs = vec![param_arc1];
