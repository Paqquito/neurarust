...

    {
        ...
        let self_vec_mut: &mut Vec<f32> = match buffer_mut_ref {
            ...
            _ => return Err(NeuraRustError::UnsupportedOperation(
                "mul_scalar_f32 expects CPU F32 buffer".to_string()
            )),
        };
        
        apply_mul_scalar_to_vec(self_vec_mut, scalar, self_shape_vec.as_slice(), self_strides_vec.as_slice(), self_offset_val, numel_self)?;
    }
    Ok(())
}
...
pub fn perform_mul_scalar_inplace_f64(current_tensor: &mut Tensor, scalar: f64) -> Result<(), NeuraRustError> {
...

    {
        ...
        let self_vec_mut: &mut Vec<f64> = match buffer_mut_ref {
            ...
            _ => return Err(NeuraRustError::UnsupportedOperation(
                "mul_scalar_f64 expects CPU F64 buffer".to_string()
            )),
        };
...
where
    T: Copy + std::ops::MulAssign + std::fmt::Debug,
{
    for i in 0..numel {
