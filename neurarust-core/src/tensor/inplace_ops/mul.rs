...
pub fn perform_mul_inplace(current_tensor: &mut Tensor, other: &Tensor) -> Result<(), NeuraRustError> {
    ...
    let other_read_guard_for_meta = other.data.read().map_err(|_| NeuraRustError::LockError {
        ...
        reason: "Failed to acquire read lock for other.data (meta) in mul_".to_string(),
    })?;
    ...
    match self_dtype {
            ...
            let self_vec_mut: &mut Vec<f32> = match buffer_enum_mut_ref {
                crate::buffer::Buffer::Cpu(ref mut cpu_buf) => match cpu_buf {
                    ...
                    _ => return Err(NeuraRustError::DataTypeMismatch { 
                        expected: DType::F32, actual: self_dtype, operation: "mul_ inplace (self buffer is not F32)".to_string()
                    }),
                },
                _ => return Err(NeuraRustError::DeviceMismatch { 
                    expected: crate::device::StorageDevice::CPU, actual: self_device_for_error, operation: "mul_ inplace (self buffer is not CPU)".to_string()
                }),
            };
        ...
        DType::F64 => {
            let self_vec_mut: &mut Vec<f64> = match buffer_enum_mut_ref {
                crate::buffer::Buffer::Cpu(ref mut cpu_buf) => match cpu_buf {
                    ...
                    _ => return Err(NeuraRustError::DataTypeMismatch { 
                        expected: DType::F64, actual: self_dtype, operation: "mul_ inplace (self buffer is not F64)".to_string()
                    }),
                },
                _ => return Err(NeuraRustError::DeviceMismatch { 
                    expected: crate::device::StorageDevice::CPU, actual: self_device_for_error, operation: "mul_ inplace (self buffer is not CPU)".to_string()
        ...
        DType::I32 | DType::I64 | DType::Bool => {
            return Err(NeuraRustError::UnsupportedOperation(
                "mul_ n'est pas supporté pour les tenseurs de type I32, I64 ou Bool".to_string())
            );
        }
    }
    Ok(())
