use crate::{
    tensor::Tensor,
    error::NeuraRustError,
    types::DType,
    buffer::{Buffer, CpuBuffer}
};
use std::sync::Arc;
use num_traits::Zero; // To check for division by zero

pub fn perform_div_scalar_inplace_f32(current_tensor: &mut Tensor, scalar: f32) -> Result<(), NeuraRustError> {
    if scalar.is_zero() {
        return Err(NeuraRustError::ArithmeticError("Division by zero scalar in div_scalar_f32.".to_string()));
    }
    if current_tensor.dtype() != DType::F32 {
        return Err(NeuraRustError::DataTypeMismatch {
            expected: DType::F32,
            actual: current_tensor.dtype(),
            operation: "in-place div_scalar_f32".to_string()
        });
    }

    if current_tensor.requires_grad() || current_tensor.grad_fn().is_some() {
        return Err(NeuraRustError::InplaceModificationError {
            operation: "div_scalar_f32".to_string(),
            reason: "Tensor requires grad or has grad_fn.".to_string()
        });
    }

    let self_shape_vec = current_tensor.shape();
    let self_strides_vec = current_tensor.strides();
    let self_offset_val;
    let numel_self;

    {
        let mut self_tensor_data_guard = current_tensor.data.write().map_err(|_| NeuraRustError::LockError{
            lock_type: "write (self)".to_string(), 
            reason: "Failed to acquire write lock for self.data in div_scalar_f32".to_string()
        })?;
        
        self_offset_val = self_tensor_data_guard.offset;
        numel_self = self_shape_vec.iter().product();

        let buffer_mut_ref: &mut Buffer = Arc::make_mut(&mut self_tensor_data_guard.buffer);

        let self_vec_mut: &mut Vec<f32> = match buffer_mut_ref {
            Buffer::Cpu(CpuBuffer::F32(arc_vec)) => Arc::make_mut(arc_vec),
            _ => return Err(NeuraRustError::UnsupportedOperation(
                "div_scalar_f32 expects CPU F32 buffer".to_string()
            )),
        };
        
        apply_div_scalar_to_vec(self_vec_mut, scalar, self_shape_vec.as_slice(), self_strides_vec.as_slice(), self_offset_val, numel_self)?;
    }
    Ok(())
}

pub fn perform_div_scalar_inplace_f64(current_tensor: &mut Tensor, scalar: f64) -> Result<(), NeuraRustError> {
    if scalar.is_zero() {
        return Err(NeuraRustError::ArithmeticError("Division by zero scalar in div_scalar_f64.".to_string()));
    }
    if current_tensor.dtype() != DType::F64 {
        return Err(NeuraRustError::DataTypeMismatch {
            expected: DType::F64,
            actual: current_tensor.dtype(),
            operation: "in-place div_scalar_f64".to_string()
        });
    }

    if current_tensor.requires_grad() || current_tensor.grad_fn().is_some() {
        return Err(NeuraRustError::InplaceModificationError {
            operation: "div_scalar_f64".to_string(),
            reason: "Tensor requires grad or has grad_fn.".to_string()
        });
    }
    
    let self_shape_vec = current_tensor.shape();
    let self_strides_vec = current_tensor.strides();
    let self_offset_val;
    let numel_self;

    {
        let mut self_tensor_data_guard = current_tensor.data.write().map_err(|_| NeuraRustError::LockError{
            lock_type: "write (self)".to_string(), 
            reason: "Failed to acquire write lock for self.data in div_scalar_f64".to_string()
        })?;
        
        self_offset_val = self_tensor_data_guard.offset;
        numel_self = self_shape_vec.iter().product();

        let buffer_mut_ref: &mut Buffer = Arc::make_mut(&mut self_tensor_data_guard.buffer);

        let self_vec_mut: &mut Vec<f64> = match buffer_mut_ref {
            Buffer::Cpu(CpuBuffer::F64(arc_vec)) => Arc::make_mut(arc_vec),
            _ => return Err(NeuraRustError::UnsupportedOperation(
                "div_scalar_f64 expects CPU F64 buffer".to_string()
            )),
        };

        apply_div_scalar_to_vec(self_vec_mut, scalar, self_shape_vec.as_slice(), self_strides_vec.as_slice(), self_offset_val, numel_self)?;
    }
    Ok(())
}

fn apply_div_scalar_to_vec<T>(
    vec_data: &mut Vec<T>, 
    scalar: T, 
    shape: &[usize],
    strides: &[usize],
    offset: usize,
    numel: usize
) -> Result<(), NeuraRustError>
where
    T: Copy + std::ops::DivAssign + Zero + std::fmt::Debug, // Zero for the check, though already done in public fn
{
    // Division by zero check is done in the calling public function to return error early.
    // If scalar could be zero here, an additional check would be needed.
    for i in 0..numel {
        let logical_coords = crate::tensor::utils::index_to_coord(i, shape);
        let mut physical_offset = offset;
        for d in 0..logical_coords.len() {
            physical_offset += logical_coords[d] * strides[d];
        }

        if physical_offset < vec_data.len() {
            vec_data[physical_offset] /= scalar;
        } else {
            return Err(NeuraRustError::IndexOutOfBounds{ index: logical_coords, shape: shape.to_vec() });
        }
    }
    Ok(())
} 